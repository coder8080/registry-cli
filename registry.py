    ...
    def bearer_request(self, method, url, auth, **kwargs):
        ...
        if res.status_code == 401:
            ...
            if "service" in oauth["bearer"]:
                request_url += "{0}service={1}".format(
                    query_separator, oauth["bearer"]["service"]
                )
                query_separator = "&"
            if "scope" in oauth["bearer"]:
                request_url += "{0}scope={1}".format(
                    query_separator, oauth["bearer"]["scope"]
                )

            if DEBUG:
                print(
                    "[debug][auth][request] Refreshing auth token: POST {0}".format(
                        request_url
                    )
                )
...
def get_auth_schemes(r, path):
    ...
    if DEBUG:
        print("[debug][funcname]: get_auth_schemes()")

    try_oauth = requests.head(
        "{0}{1}".format(r.hostname, path), verify=not r.no_validate_ssl
    )
...
class Registry:
    ...
    def delete_tag(self, image_name, tag, dry_run, tag_digests_to_ignore):
        ...
        delete_result = self.send(
            "/v2/{0}/manifests/{1}".format(image_name, tag_digest), method="DELETE"
        )
    ...
    def get_image_age(self, image_name, image_config):
        ...
        if "bearer" in self.auth_schemes:
            ...
            (response, self.HEADERS["Authorization"]) = self.http.bearer_request(
                ...
                "{0}{1}".format(
                    self.hostname,
                    "/v2/{0}/blobs/{1}".format(image_name, image_config["digest"]),
                ),
                auth=(
                    ("", "")
                    if self.username in ["", None]
...
for more detail on garbage collection read here:
   https://docs.docker.com/registry/garbage-collection/
    ...
    parser.add_argument(
        "-d",
        "--delete",
        help=("If specified, delete all but last {0} tags of all images").format(
            CONST_KEEP_LAST_VERSIONS
        ),
        action="store_const",
        default=False,
        const=True,
    ...
    parser.add_argument(
        "-n",
        "--num",
        help=("Set the number of tags to keep({0} if not set)").format(
            CONST_KEEP_LAST_VERSIONS
        ),
    ...
    parser.add_argument(
        "--dry-run",
        help=("If used in combination with --delete,then images will not be deleted"),
        action="store_const",
        default=False,
        const=True,
    )
    ...
    parser.add_argument(
        "--order-by-date",
        help=(
            "Orders images by date instead of by tag name."
    ...
    parser.add_argument(
        "--plain",
        help=(
            "Turn plain output, one image:tag per line."
...
def delete_tags(registry, image_name, dry_run, tags_to_delete, tags_to_keep):
    ...
    if tags_to_keep:
        ...
        for tag in tags_to_keep:
            ...
            if digest is None:
                print(
                    "Tag {0} does not exist for image {1}. Ignore here.".format(
                        tag, image_name
                    )
                )
                continue
...
def delete_tags_by_age(registry, image_name, dry_run, hours, tags_to_keep):
    ...
    for tag in image_tags:
        ...
        if parse(image_age).astimezone(tzutc()) < dt.now(tzutc()) - timedelta(
            hours=int(hours)
        ):
            print("will be deleted tag: {0} timestamp: {1}".format(tag, image_age))
...
def get_newer_tags(registry, image_name, hours, tags_list):
    def newer(tag):
        ...
        if parse(image_age).astimezone(tzutc()) >= dt.now(tzutc()) - timedelta(
            ...
            print("Keeping tag: {0} timestamp: {1}".format(tag, image_age))
            return tag
...
def get_datetime_tags(registry, image_name, tags_list, plain):
    def newer(tag):
        ...
        if image_age == []:
            print("timestamp not found")
            return None
        return {"tag": tag, "datetime": parse(image_age).astimezone(tzutc())}
...
def get_ordered_tags(registry, image_name, tags_list, order_by_date=False):
    if order_by_date:
        tags_date = get_datetime_tags(registry, image_name, tags_list, False)
        sorted_tags_by_date = sorted(tags_date, key=lambda x: x["datetime"])
...
def main_loop(args):
    ...
    registry = Registry.create(
        args.host, args.login, args.no_validate_ssl, args.digest_method
    )
    ...
    for image_name in image_list:
        ...
        if args.order_by_date:
            tags_list = get_ordered_tags(
                registry, image_name, all_tags_list, args.order_by_date
            )
        else:
        ...
        for tag in tags_list:
            ...
            if args.layers:
                for layer in registry.list_tag_layers(image_name, tag):
                    if "size" in layer:
                        print(
                            "    layer: {0}, size: {1}".format(
                                layer["digest"], layer["size"]
                            )
                        )
                    else:
                        print("    layer: {0}".format(layer["blobSum"]))
        ...
        if args.keep_by_hours:
            keep_tags.extend(
                get_newer_tags(registry, image_name, args.keep_by_hours, tags_list)
            )
        ...
        if args.delete or args.delete_all:
            if args.delete_all:
                ...
                ordered_tags_list = get_ordered_tags(
                    registry, image_name, tags_list, args.order_by_date
                )
            ...
            delete_tags(
                registry, image_name, args.dry_run, tags_list_to_delete, keep_tags
            )

        # delete tags by age in hours
        if args.delete_by_hours:
            delete_tags_by_age(
                registry, image_name, args.dry_run, args.delete_by_hours, keep_tags
            )
